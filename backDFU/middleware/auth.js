...
async function autenticarFirebase(req, res, next) {
  ...
  try {
    const decoded = await admin.auth().verifyIdToken(token)  // Verifica o token com o Firebase Admin SDK

    req.userId = decoded.uid;  // Armazena o ID do usuário (médico) no request
